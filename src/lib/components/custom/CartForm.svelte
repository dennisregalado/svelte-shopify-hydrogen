...
<script lang="ts">
	...
	let { children, action, inputs, route = '/cart' }: CartFormProps = $props();
...
<form
	...
	action={route || ''}
>
	<input type="hidden" name={INPUT_NAME} value={JSON.stringify({ action, inputs })} />
