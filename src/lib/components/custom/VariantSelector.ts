...
export function VariantSelector({
	...
	const selectedVariantOptions = selectedVariant
		? selectedVariant?.selectedOptions?.reduce<Record<string, string>>((selectedValues, item) => {
				...
				for (let value of option.optionValues!) {
					...
					optionsWithOnlyOneValue.forEach((option) => {
						if (option.optionValues![0]!.name)
							clonedSearchParams.set(option.name!, option.optionValues![0]!.name);
					});
 ...
 * ```
 **/
export const getSelectedProductOptions: GetSelectedProductOptions = (request) => {
